...
export const metadata = {
  title: 'AGENTS.md | Create Rust App',
...
## 3. AI Execution Protocol (Rust Feature Work)

When asked to add/modify API behavior:
1. Locate the relevant Rust module (src/ feature modules)
...
5. After code: list follow-up validation steps (cargo fmt, cargo clippy, cargo test)

## 4. Guardrails (Must Enforce)

- Do NOT fabricate file paths, settings keys, or package versions
- Do NOT invent crates.io crate versions or import paths
- Do NOT nest modules under src/features/ (keep the flat module layout)
- ALWAYS flag new Cargo.toml dependencies for human confirmation
- ALWAYS surface ownership / concurrency tradeoffs when changing systems code
...
- cargo test coverage added or deferred with reason
...
Refuse if: asked to bypass validation, skip cargo clippy, or invent undocumented crates.io deps.
...
- cargo fmt / cargo clippy / cargo test status
- Suggested manual QA steps
- Deferred items (tests, docs)

---
Maintained automatically by create-rust-app web-server template provisioning.
              ...
              <Card className="backdrop-blur-sm bg-card/60">
                ...
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  ...
                  <ul className="list-disc pl-5 space-y-1">
                    ...
                    <li>
                      Include CI scripts or task runners (e.g. <code>cargo test</code>,{' '}
                      <code>cargo fmt --check && cargo clippy</code>) if not obvious
                    </li>
