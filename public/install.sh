#!/usr/bin/env sh
# install.sh — install create-rust-app from GitHub Releases (curl|sh / wget|sh).
#
# Usage:
#   curl -fsSL https://create-awesome-rust-app.vercel.app/install.sh | sh
#   wget -qO- https://create-awesome-rust-app.vercel.app/install.sh | sh
#
# Env:
#   CRA_VERSION=0.1.0              # semver without tag prefix
#   CRA_RELEASE_TAG=create-rust-app@0.1.0
#   CRA_INSTALL_DIR=$HOME/.local/bin
#   CRA_DRY_RUN=1                  # print actions only
#   CRA_NO_MODIFY_PATH=1           # reserved; this script never edits shell rc files
#   CRA_REPO=Create-Rust-App/create-rust-app
#
# Installs create-rust-app and a create-awesome-rust-app symlink.
set -eu

REPO="${CRA_REPO:-Create-Rust-App/create-rust-app}"
BIN_NAME="create-rust-app"
ALIAS_NAME="create-awesome-rust-app"
INSTALL_DIR="${CRA_INSTALL_DIR:-${HOME}/.local/bin}"
DRY_RUN="${CRA_DRY_RUN:-0}"
...
resolve_tag() {
	if [ -n "${CRA_RELEASE_TAG:-}" ]; then
		printf '%s\n' "$CRA_RELEASE_TAG"
		return
	fi
	if [ -n "${CRA_VERSION:-}" ]; then
		printf 'create-rust-app@%s\n' "$CRA_VERSION"
		return
	fi
	# Prefer releases/latest when it is a CRA tag; otherwise scan recent releases.
	...
	_tag=$(printf '%s' "$_list_json" | sed -n 's/.*"tag_name"[[:space:]]*:[[:space:]]*"\(create-rust-app@[^\"]*\)".*/\1/p' | head -n1)
	[ -n "$_tag" ] || die "could not resolve a create-rust-app@* release tag"
	printf '%s\n' "$_tag"
}
...
main() {
	...
	info "Create Rust App installer"
	info "  tag:      ${_tag}"
	info "  asset:    ${_asset}"
